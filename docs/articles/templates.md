...
若你是透過 `SpreadsheetDocument.FromJson(...)` 建立文件，目前 JSON 支援 `Grid`、`RecordSet` 與 `DataTable` 三種 template 型別。完整 JSON 欄位請參考 [SpreadsheetDocument](spreadsheet-document.md) 的 JSON 章節。
