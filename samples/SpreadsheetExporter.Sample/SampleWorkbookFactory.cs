...
namespace CloudyWing.SpreadsheetExporter.Sample;

/// <summary>
/// Creates the workbook documents used by the sample application.
/// </summary>
public static class SampleWorkbookFactory {
    /// <summary>
    /// Creates the fluent API sample workbook using the current local timestamp.
    /// </summary>
    /// <returns>The configured sample workbook.</returns>
    public static SpreadsheetDocument CreateFluentDocument() {
        return CreateFluentDocument(DateTimeOffset.Now);
    }

    /// <summary>
    /// Creates the fluent API sample workbook using the specified generated timestamp.
    /// </summary>
    /// <param name="generatedAt">The generated timestamp displayed in the overview sheet.</param>
    /// <returns>The configured sample workbook.</returns>
    public static SpreadsheetDocument CreateFluentDocument(DateTimeOffset generatedAt) {
        ...
        document
            ...
            .SetColumnWidth(2, 20)
            .SetColumnWidth(3, 30)
            .AddTemplate(CreateOverviewTemplate(generatedAt));
    ...
    /// <summary>
    /// Creates the JSON template sample workbook.
    /// </summary>
    /// <returns>The configured sample workbook.</returns>
    public static SpreadsheetDocument CreateJsonDocument() {
        return SpreadsheetDocument.FromJson(CreateJsonTemplate());
    }

    /// <summary>
    /// Creates the JSON template string used by the sample application.
    /// </summary>
    /// <returns>The JSON template string.</returns>
    ...
    private static GridTemplate CreateOverviewTemplate(DateTimeOffset generatedAt) {
    ...
    private static RecordSetTemplate<SampleOrder> CreateOrdersTemplate(IEnumerable<SampleOrder> orders) {
    ...
    private static ISheetTemplate CreateProductsTemplate(DataTable products) {
        CellStyle titleStyle = new(
            HorizontalAlignment: HorizontalAlignment.Center,
            HasBorder: true,
    ...
    private static IEnumerable<SampleOrder> GetSampleOrders() => [
    ...
    private static DataTable GetSampleProducts() {
        DataTable table = new();
    ...
    private sealed record SampleOrder(int OrderId, string Customer, string Region, decimal Amount, int Qty);
}
